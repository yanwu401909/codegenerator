...
public class Table {
	...
	public static String removeTableSqlNamePrefix(String sqlName) {
		boolean removePrefix = GeneratorProperties.getRequiredBoolean("removePrefix");
		if(removePrefix){
			sqlName = filter.removeTableSqlNamePrefix(sqlName);
		}else{
   ...
   public Column getRequiredColumnBySqlName(String sqlName) {
	...
	public interface TablePrefixFilter {
		public String removeTableSqlNamePrefix(String name);
	}
	
	/**
	 * tableName 表前缀处理
	 */
	 static TablePrefixFilter filter = new TablePrefixFilter(){
		public String removeTableSqlNamePrefix(String name) {
			if(name != null){
