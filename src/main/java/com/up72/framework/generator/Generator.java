...
import com.up72.framework.generator.util.AntPathMatcher;
import com.up72.framework.generator.util.BeanHelper;
import com.up72.framework.generator.util.FileFilter;
import com.up72.framework.generator.util.FileHelper;
import com.up72.framework.generator.util.FreemarkerHelper;
import com.up72.framework.generator.util.GLogger;
import com.up72.framework.generator.util.GeneratorException;
import com.up72.framework.generator.util.IOHelper;
import com.up72.framework.generator.util.StringHelper;
...
//		if(outRootDir == null) throw new IllegalStateException("'outRootDir' property must be not null.");
     ...
     * @throws Exception
     */
	public Generator generateBy(Map templateModel,Map filePathModel,FileFilter filter) throws Exception {
		processTemplateRootDirs(templateModel, filePathModel,false,filter);
		return this;
	}
	 ...
	 * @throws Exception
	 */
    public Generator deleteBy(Map templateModel,Map filePathModel,FileFilter filter) throws Exception {
    	processTemplateRootDirs(templateModel, filePathModel,true,null);
    	return this;
    }	
	
	private void processTemplateRootDirs(Map templateModel,Map filePathModel,boolean isDelete,FileFilter filter) throws Exception {
	    if(StringHelper.isBlank(getOutRootDir())) throw new IllegalStateException("'outRootDir' property must be not null.");
		...
		for(int i = 0; i < this.templateRootDirs.size(); i++) {
			...
			List<Exception> exceptions = scanTemplatesAndProcess(templateRootDir,templateModel,filePathModel,isDelete, filter);
			ge.addAll(exceptions); 
		}
		if(!ge.exceptions.isEmpty()) throw ge;
	}
	
	private List<Exception> scanTemplatesAndProcess(File templateRootDir, Map templateModel,Map filePathModel,boolean isDelete, FileFilter filter) throws Exception {
		...
		List srcFiles = FileHelper.searchAllNotIgnoreFile(templateRootDir, filter);
